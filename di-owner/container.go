...
// 注入依赖
func (p *Container) Ensure(instance interface{}) (interface{}, error) {
	...
	for i := 0; i < elemType.NumField(); i++ { // 遍历字段
		...
		if diInstance == nil {
			return nil, errors.New(diName + " dependency not found")
		}
...
func (p *Container) String() string {
	...
	for name, item := range p.singletons {
		it := item
		line := fmt.Sprintf("  %s: %x %s", name, &it, reflect.TypeOf(it).String())
